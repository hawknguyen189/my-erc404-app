...
const AsteroidMap = () => {
  ...
  galaxyImage.onerror = () =>
    console.error(
      "Failed to load galaxy-map.png. Check path:",
      galaxyImage.src
    );
  ...
  useEffect(() => {
    ...
    const drawMap = () => {
      ...
      if (
        galaxyData.planet &&
        typeof galaxyData.planet.x === "number" &&
        typeof galaxyData.planet.y === "number"
      ) {
        ctx.beginPath();
        ctx.arc(galaxyData.planet.x, galaxyData.planet.y, 20, 0, Math.PI * 2);
        ctx.fillStyle =
          galaxyData.planet.owner === walletAddress ? "green" : "blue";
        ctx.fill();
        ...
        ctx.font = "12px Arial";
        ctx.fillText(
          galaxyData.planet.name || "Unnamed",
          galaxyData.planet.x - 10,
          galaxyData.planet.y - 25
        );
      }
      ...
      if (
        galaxyData.asteroid &&
        typeof galaxyData.asteroid.x === "number" &&
        typeof galaxyData.asteroid.y === "number"
      ) {
        ctx.beginPath();
        ctx.arc(
          galaxyData.asteroid.x,
          galaxyData.asteroid.y,
          10,
          0,
          Math.PI * 2
        );
        ctx.fillStyle =
          galaxyData.asteroid.owner === walletAddress ? "green" : "gray";
        ctx.fill();
        ctx.strokeStyle = "white";
        ctx.stroke();
        ctx.fillStyle = "white";
        ctx.font = "10px Arial";
        ctx.fillText(
          galaxyData.asteroid.name || "Unnamed",
          galaxyData.asteroid.x - 10,
          galaxyData.asteroid.y - 15
        );
      }
    ...
    const handleClick = (event) => {
          ...
          if (walletAddress && !data.owner) {
            (async () => {
              try {
                ...
                console.error(
                  `Error ${type === "planet" ? "staking" : "claiming"}:`,
                  error
                );
              }
            })();
      ...
      if (
        ...
        const distance = Math.sqrt(
          (x - galaxyData.planet.x) ** 2 + (y - galaxyData.planet.y) ** 2
        );
        if (distance < 20) {
          performAction("planet", galaxyData.planet, () =>
            stakeResources({ args: [galaxyData.planet.id, 10] })
          );
        }
      }
      ...
      if (
        ...
        const distance = Math.sqrt(
          (x - galaxyData.asteroid.x) ** 2 + (y - galaxyData.asteroid.y) ** 2
        );
        if (distance < 10) {
          performAction("asteroid", galaxyData.asteroid, () =>
            stakeResources({ args: [galaxyData.asteroid.id, 5] })
          );
        }
      }
    };
